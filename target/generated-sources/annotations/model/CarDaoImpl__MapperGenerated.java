 ...
 */
public class CarDaoImpl__MapperGenerated extends DaoBase implements CarDao {
  private static final Logger LOG = LoggerFactory.getLogger(CarDaoImpl__MapperGenerated.class);
  ...
  private CarDaoImpl__MapperGenerated(MapperContext context, CarHelper__MapperGenerated carHelper,
  ...
  public Car findById(Integer id) {
    ...
    return executeAndMapToSingleEntity(boundStatement, carHelper);
  }

  @Override
  public void save(Car car) {
    BoundStatementBuilder boundStatementBuilder = saveStatement.boundStatementBuilder();
    carHelper.set(car, boundStatementBuilder, NullSavingStrategy.DO_NOT_SET);
  ...
  public void delete(Car car) {
    ...
    boundStatementBuilder = boundStatementBuilder.set("id", car.getId(), Integer.class);

    BoundStatement boundStatement = boundStatementBuilder.build();
    execute(boundStatement);
  }

  public static CompletableFuture<CarDao> initAsync(MapperContext context) {
    ...
    try {
      ...
      SimpleStatement findByIdStatement_simple = carHelper.selectByPrimaryKeyParts(1).build();
      ...
      // Prepare the statement for `save(model.Car)`:
      SimpleStatement saveStatement_simple = carHelper.insert().build();
      LOG.debug("[{}] Preparing query `{}` for method save(model.Car)",
          context.getSession().getName(),
      ...
      // Prepare the statement for `update(model.Car)`:
      SimpleStatement updateStatement_simple = SimpleStatement.newInstance(((DefaultUpdate)carHelper.updateByPrimaryKey()).asCql());
      LOG.debug("[{}] Preparing query `{}` for method update(model.Car)",
          context.getSession().getName(),
      ...
      // Prepare the statement for `delete(model.Car)`:
      SimpleStatement deleteStatement_simple = carHelper.deleteByPrimaryKeyParts(1).build();
      LOG.debug("[{}] Preparing query `{}` for method delete(model.Car)",
          context.getSession().getName(),
      ...
      return CompletableFutures.allSuccessful(prepareStages)
          .thenApply(v -> (CarDao) new CarDaoImpl__MapperGenerated(context,
              carHelper,
  ...
  public static CarDao init(MapperContext context) {
