 ...
 *         hot end of the LRU list. When an off-heap cache is configured, the
 *         eviction of LNs in this manner occurs independently in both caches.
 *     </li>
 *     <li>
...
public enum CacheMode {
     ...
     *     the main cache in order to avoid a net change to the cache,
     *     according to the rules below.</li>
     *
     *     <li>If the record's LN was not present in the main cache prior to
     *     the operation, then the LN is not added to the main cache by the
     *     operation. The LN will not be added to, or removed from, the
     *     off-heap cache.</li>
     *
    ...
    /**
     * The record's LN is not added to the main cache by the operation, it is
     * evicted from main cache if it was previously present, and the
     ...
     * <p>Specifically:
     * <ul>
     *     <li>The record's LN is not added to the main cache and it will be
     *     evicted if previously present. The LN will be added to the off-heap
     *     cache, if it is not already present and an off-heap cache is
     *     configured.
     * </ul>
     *
     * @since 3.3.98
     ...
     *         main cache after the operation. The BIN will be added to the
     *         off-heap cache, if it is not already present and an off-heap
     *         cache is configured. The BIN will be placed at the hot end of
     *         its off-heap LRU list.</li>
     *
     *         <li>When the BIN is dirty and an off-heap cache is
     *         <em>not</em> configured, the BIN will not be evicted from the
     *         main cache and will be moved to the hot end of its main cache
