...
class CHeapAllocator implements OffHeapAllocator {
    ...
    public boolean equals(byte[] buf,
        ...
        if (CHECK_BOUNDS) {
            ...
            if (memOff < 0 || memOff > memSize) {
                throw new IndexOutOfBoundsException(
                    "memOff=" + memOff +
                        " memSize=" + memSize);
            }
            if (bufOff < 0 || bufOff + compareLen > buf.length) {
                throw new IndexOutOfBoundsException(
                    "bufOff=" + bufOff +
                        " bufSize=" + buf.length +
        ...
        if (compareLen != memSize) {
            return false;
        }

        final long memAddr = memId + memOff + SIZE_BYTES;

        for (int i = 0; i < compareLen; i += 1) {
            if (unsafe.getByte(memAddr + i) != buf[bufOff + i]) {
                return false;
            }
        }
