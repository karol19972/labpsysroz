...
public class Cleaner implements DaemonRunner, EnvConfigObserver {
    ...
    private String getLogSizeStatsFeederName(final String name) {
        if (name.startsWith(FEEDER_NAME + "-")) {
            final String nameIdPair = name.substring(FEEDER_NAME.length() + 1);
            return NameIdPair.getNameFromString(nameIdPair);
        }
        return null;
    }
     ...
     * regain disk space.  If no bytes were recovered and this is the master,
     * then pick a lagging replica to kill and try again.  Should only be
     * called when holding the manageDiskUsageLock.
     */
    private long truncateVLSNIndexAndDelete(
        ...
        /*
         * If no bytes have been reclaimed yet and this is the master then
         * look for a lagging replica to kill to allow bytes to be reclaimed.
         */
        if (bytesNeeded == originalBytes && env.getIsMaster()) {
            LoggerUtils.info(logger, env,
                "Attempting to kill a replica to enable deletion of reserved" +
                " files and prevent an out-of-disk condition.");
            /*
             ...
             */
            String feederToKill = null;
            long replicaBytesSaved = 0;
            long otherBytesSaved = 0;
            String otherEntity = null;
            for (Map.Entry<String, Long> entity :
                logSizeStats.protectedSizeMap.entrySet()) {
                final String feederName =
                    getLogSizeStatsFeederName(entity.getKey());
                if (entity.getValue() > replicaBytesSaved &&
                    feederName != null) {
                    feederToKill= feederName;
                    replicaBytesSaved = entity.getValue();
                } else if (entity.getValue() > otherBytesSaved && 
                    feederName == null) {
                    otherBytesSaved = entity.getValue();
                    otherEntity = entity.getKey();
                }
            }
            if (feederToKill == null) {
                LoggerUtils.info(logger, env, "No replica found to kill.");
                return bytesNeeded;
            }
            if (otherBytesSaved >= replicaBytesSaved) {
                LoggerUtils.info(logger, env, "Killing a replica will not" +
                    " free enough bytes as other sources are protecting" + 
                    " files. Replica=" + feederToKill +
                    ", replicaProtectedBytes=" + replicaBytesSaved +
            ...
            final Feeder feeder = feederManager.getFeeder(feederToKill);
            // May have already been shut down.
            if (feeder == null) {
                LoggerUtils.info(logger, env,
                    feederToKill + " not found in active Feeder list");
                return bytesNeeded;
            }
