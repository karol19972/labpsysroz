...
public class FileProcessor extends DaemonThread {
    ...
    private void processLN(
        ...
        try {
            ...
            /*
             * If the BIN was not resident, move it off-heap immediately
             * to avoid impacting the main cache.
             */
    ...
    private LNInfo processFoundLN(
        ...
        try {
            ...
            if (db.getId().equals(DbTree.ID_DB_ID)) {
                ...
                /* TODO: why dirty BIN when LN will be evicted? */
                bin.setDirty(true);
                nLNsMarkedThisRun++;
    ...
    private void processBINDelta(
        ...
        try {
             ...
             *
             * If the BIN is off-heap, we can dirty it without moving it to the
             * main cache.
             */
            if (result.parent.getOffHeapBINId(result.index) >= 0) {
                result.parent.setOffHeapBINDirty(result.index, true);
            } else {
    ...
    private boolean processIN(
        ...
        try {
            ...
            final int ohBinId = result.parent.getOffHeapBINId(result.index);
            final OffHeapCache.BINInfo ohBin;

            if (ohBinId >= 0) {
                ohBin =
                    envImpl.getOffHeapCache().getBINInfo(envImpl, ohBinId);
            } else {
                ohBin = null;
            }
             ...
             */
            if (treeLsn == logLsn) {
                if (ohBin != null && !ohBin.isBINDelta()) {
                    migrateOffHeapBIN(
                        result.parent, result.index, ohBinId, ohBin);
                    return true;
                }
                ...
                if (in != null) {
                    ...
                    /*
                     * Off-heap BIN is a delta since a full OH BIN is handled
                     * above. It must be dirty for the reason described above
                     * and therefore we must mutate it to a full BIN.
                     */
                    assert ohBin.isBINDelta();
                    assert parent.isOffHeapBINDirty(result.index);

                    final BIN bin = (BIN) parent.loadIN(
                    ...
                    /* To avoid a fetch, use the inClone for mutation. */
                    bin.mutateToFullBIN(
                        (BIN) inClone, false /*leaveFreeSlot*/,
                        logLsn, wholeEntry);
                    in = bin;
            ...
            BIN bin;
            if (ohBin != null) {
                treeLsn = ohBin.getFullBINLsn();
                bin = null;
            } else {
                bin = (BIN) parent.fetchIN(
                    result.index, CacheMode.UNCHANGED);
    ...
    private void migrateOffHeapBIN(final IN parent,
                                   final int index,
                                   final int ohBinId,
                                   final OffHeapCache.BINInfo ohBin) {
        assert !ohBin.isBINDelta();
        ...
        /*
         * Dirty the off-heap BIN. Checkpoint or eviction will write it out.
         *
         * Prohibit the next delta, since the original version must be
         * made obsolete.
         *
         * Compression (to reclaim space for expired slots, including dirty
         * slots), requires materializing the BIN, so this is deferred to
         * checkpoint time.
         */
        parent.setOffHeapBINDirty(index, true);
        ohBin.setProhibitNextDelta();
        envImpl.getOffHeapCache().updateFlags(envImpl, ohBinId, ohBin);
    }
