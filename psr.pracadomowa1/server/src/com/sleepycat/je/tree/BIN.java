...
public class BIN extends IN {
    ...
    protected long printMemorySize() {
        ...
        final long binTotal = inTotal +
            vlsnCacheOverhead + expirationOverhead +
            modTimeOverhead + offHeapLNIdOverhead;

        System.out.format(
            "BIN: %d vlsns: %d expiration: %d" +
                " modTimeOverhead: %d offHeapLNIds: %d %n",
     ...
     *
     * Note that evicting deferred-write LNs may require logging them, which
     * will mark this BIN dirty. Compression of deleted slots will also mark
     * the BIN dirty.
     *
    ...
    private long evictLNInternal(int index, boolean ifFetchedCold)
        ...
        /*
         * Whenever we remove an LN from the main cache, add it to the off-heap
         * cache. Note that this does not occur when the LN is embedded in the
         * BIN, since getTarget would have returned null further above.
         */
        final OffHeapCache ohCache = getOffHeapCache();
        if (ohCache.isEnabled()) {
    ...
    private BIN fetchFullBIN(DatabaseImpl dbImpl) {
        ...
        if (lsn == DbLsn.NULL_LSN) {
            throw unexpectedState(
                makeFetchErrorMsg("NULL_LSN as LastFullLsn", lsn, -1));
        }
        checkLsnInActiveFile(envImpl, lsn, -1);
