...
public class IN extends Node implements Comparable<IN>, LatchContext {
    ...
    public final void updateLRU(CacheMode cacheMode) {
        ...
        case EVICT_BIN:
            /*
             * We want to store the BIN off-heap even if it was fetched cold.
             * See OffHeapCache.storeEvictedBIN.
             */
            setFetchedCold(false);
            setFetchedColdOffHeap(false);
            break;
    ...
    public void setOffHeapBINDirty(int idx, boolean val) {
        if (val) {
    ...
    private IN fetchINWithNoLatch(
        ...
        checkLsnInActiveFile(envImpl, lsn, idx);
    ...
    public IN loadIN(int idx, CacheMode cacheMode) {
        ...
        try {
            final ByteBuffer ohBytes = ohCache.getBINBytes(this, idx);
            if (ohBytes == null) {
                return null;
            }
    ...
    public final LN fetchLN(int idx, CacheMode cacheMode) {
        ...
        if (ln == null) {
            ...
            try {
                ...
                ln = ohCache.loadLN(bin, idx, cacheMode);
    ...
    /**
     * Construct exception essage for fetch errors.
     *
    ...
    void checkLsnInActiveFile(
        ...
        /* LSNs of dirty nodes in temporary DBs are never fetched. */
        if (getDatabase().isTemporary()) {
            if (idx < 0) {
                if (getDirty()) {
                    return;
                }
            } else {
                final Node node = getTarget(idx);
                if (node != null) {
                    if (isBIN() ?
                        ((LN) node).isDirty() :
                        ((IN) node).getDirty()) {
        ...
        final String logMsg = makeFetchErrorMsg(
            "Active LSN refers to reserved file.", lsn, idx);
        ...
        if (verifyContext == null) {
            if (envImpl.isTestMode()) {
                ...
                LoggerUtils.envLogMsg(Level.SEVERE,
                    envImpl, LoggerUtils.getStackTrace(e));
            ...
            verifyContext.recordError(
                new VerifyError(getCleanedNodeProblem(lsn),
                    logMsg, lsn));
        }
    }
    ...
    private void handleCorruption(
        ...
        final String logMsg = makeFetchErrorMsg(msg, lsn, idx);
    ...
    private LN handleCleanedLN(
        ...
        handleCorruption(getEnv(), lsn, idx,
    ...
    int insertEntryInternal(
        ...
        if (embData == null || databaseImpl.isDeferredWriteMode()) {
    ...
    final IN splitInternal(
        ...
        try {
            ...
            LogItem myNewItem = optionalLogProvisionalNoCompress(parent);
            ...
            if (!Tree.LOG_SPLITS) {
                ...
                parentLsn = parent.optionalLog().lsn;
            } else {
                LogItem parentItem =
                    parent.optionalLogProvisional(grandParent);
     ...
     */
    private void prepareForSlotReuse(int idx) {
    ...
    /**
     * Returns whether any child is non-null in the main cache.
     */
    ...
    private void setLastLsnsAndSizes(long lsn, WholeEntry wholeEntry) {
        ...
        if (isBINDelta()) {
            ...
            } else if (entry instanceof OldBINDeltaLogEntry) {
                final OldBINDeltaLogEntry oldBinEntry =
                    (OldBINDeltaLogEntry) entry;
                lastFullLsn = oldBinEntry.getMainItem().getLastFullLsn();
                lastFullLogSize = 0;
            } else {
                throw unexpectedState();
            }
            ...
            if (child != null) {
                ...
                try {
                    if (child.getDirty()) {
                        ...
                        updateEntry(i, logItem.lsn, NULL_VLSN, logItem.size);
                    }
                } finally {
                    child.releaseLatch();
                }
    ...
    final LogItem optionalLog() {

        if (databaseImpl.isDeferredWriteMode()) {
            return deferredWriteLogItem();
        } else {
            return logInternal(
                this, null, false /*allowDeltas*/, true /*allowCompress*/,
                Provisional.NO, false /*backgroundIO*/, null /*parent*/);
    ...
    LogItem optionalLogProvisional(IN parent) {
        return optionalLogProvisional(parent, true /*allowCompress*/);
    }

    LogItem optionalLogProvisionalNoCompress(IN parent) {
        return optionalLogProvisional(parent, false /*allowCompress*/);
    }

    private LogItem optionalLogProvisional(IN parent, boolean allowCompress) {
    ...
    public static LogItem logEntry(
    ...
    /**
     * For "optional" logging methods in a deferred-write DB, we simply
     * return the current LSN.
     */
    private LogItem deferredWriteLogItem() {
        final LogItem item = new LogItem();
        item.lsn = getLastLoggedLsn();
        return item;
    }
     ...
     */
    private static LogItem logInternal(
        final IN node,
        INLogEntry<?> logEntry,
        ...
        if (countObsoleteNow) {
            ...
            if (node != null) {
                params.packedObsoleteInfo = node.provisionalObsolete;
         ...
         */
        if (node != null && !envImpl.isReadOnly()) {
            node.setDirty(false);
        }
        ...
        if (bin != null) {
            ...
            node.setLastFullLsnAndSize(item.lsn, item.size);
        }

        final Evictor evictor = envImpl.getEvictor();

        if (node != null && evictor.useDirtyLRUSet()) {
    ...
    /**
     * WARNING: In the case of BINs this method is used not only for reading
     * from the log but also for off-heap caching. Therefore, this method
     * should not have side effects unless the clearDirtyBits param is true
     * or an older log version is passed (off-heap caching uses the current
     * version).
     */
