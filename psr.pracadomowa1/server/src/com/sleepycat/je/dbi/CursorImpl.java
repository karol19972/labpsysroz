...
public class CursorImpl implements Cloneable {
    ...
    /**
     * Determine whether to cache an LN in the main cache as part of an
     * operation.
     *
     ...
     * BIN slot target should be set to null if not already null), and the LN
     * should be cached off-heap if an off-heap cache is configured.</p>
     *
    ...
    private static boolean shouldCacheLN(
        ...
        /*
         * For DW DBs we must cache a dirty LN even if the data is stored in
         * the BIN, because no logrec is generated, and as a result, the LN
         * must be in the tree so that a logrec will be generated when a db
         * sync() occurs later (that logrec is needed for crash recovery,
         * because BINs are not replayed during crash recovery).
         *
         * TODO: Can we avoid caching an embedded LN for temporary DBs?
         */
        if (dbImpl.isDeferredWriteMode() &&
            (isInsertion || isUpdate ||
                (treeLn != null && treeLn.isDirty()))) {
            return true;
        }
        /*
    ...
    /**
     * Returns whether an LN will be stored off-heap after a write.
     */
    static boolean shouldStoreOffheapLNAfterWrite(
        final DatabaseImpl dbImpl,
        final LN ln,
        final boolean isInsertion,
        final CacheMode cacheMode) {

        /* Check deleted first. Cannot call getDataSize for a deleted LN. */
        if (ln.isDeleted()) {
            return false;
        }

        final boolean embeddedLN = shouldEmbedLN(dbImpl, ln.getDataSize());

        final boolean shouldCache = shouldCacheLN(
            dbImpl, ln, embeddedLN, isInsertion, !isInsertion, cacheMode);

        return dbImpl.getEnv().getOffHeapCache().shouldStoreLNAfterWrite(
            dbImpl, shouldCache, embeddedLN);
    }

    /**
     * If LN will not be cached in main, store it in the off-heap cache.
     *
     * <p>Call this method before calling evictLN to make use of the
     * preprocessor's off-heap LN. Should not be called for a deletion.</p>
     */
    private void storeOffHeapLNAfterWrite(
        ...
        if (ohCache.shouldStoreLNAfterWrite(dbImpl, shouldCache, embeddedLN)) {
            ohCache.storeLN(
                bin, index,
                (preprocessor != null) ? preprocessor.takeOffHeapLnId() : 0,
                treeLn, data, vlsn, modificationTime);
        }
    }
     ...
     *
     * <p>Blind deletion is not used for deferred-write dbs because we
     * haven't done the analysis to determine whether this will work.</p>
     *
        ...
        try {
            ...
            if (index >= 0) {
                ...
                return deleteCurrentRecord(
                    modificationTime, repContext, true /*latched*/);
            }
            ...
            try {
                ...
                final LogItem logItem = ln.optionalLog(
                ...
                return DbInternal.makeResult(
                    ...
                    0L /*modificationTime*/, false /*tombstone*/);
    ...
    public OperationResult deleteCurrentRecord(ReplicationContext repContext) {
        return deleteCurrentRecord(
            0L /*modificationTime*/, repContext, false /*latched*/);
    }
    ...
    public OperationResult deleteCurrentRecord(long modificationTime,
        ...
        try {
            ...
            return DbInternal.makeResult(
                expiration, expirationInHours, false /*update*/,
                modificationTime, false /*tombstone*/);
    ...
    private Pair<LockStanding, OperationResult> insertRecordInternal(
        ...
        try {
            logItem = loggingLn.optionalLog(
                ...
                0 /*currSize*/, true/*isInsertion*/, writeParams.repContext);
        } finally {
        ...
        storeOffHeapLNAfterWrite(
            shouldCache, newEmbeddedLN, writeParams.preprocessor, data,
            treeLn, vlsn, writeParams.modificationTime);
    ...
    private OperationResult updateRecordInternal(
        ...
        writeParams.setOldTombstone(bin.isTombstone(index));
        ...
        if (writeParams.updateExpiration) {
        ...
        logItem = loggingLn.optionalLog(
            ...
            writeParams.repContext);

        final long vlsn = loggingLn.getVLSNSequence();

        /* Must store preprocessor's off-heap LN before calling evictLN. */
        storeOffHeapLNAfterWrite(
            shouldCache, newEmbeddedLN, writeParams.preprocessor, data,
            treeLn, vlsn, writeParams.modificationTime);
    ...
    public OperationResult getCurrent(
        ...
        return DbInternal.makeResult(
            bin.getExpiration(index), bin.isExpirationInHours(), false,
            (ln != null) ?
