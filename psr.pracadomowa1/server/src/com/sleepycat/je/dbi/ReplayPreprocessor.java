 ...
 *     </li>
 *
 *     <li>If we will cache the LN off-heap, an off-heap LN is allocated here
 *     and is used during replay. This avoids an expensive 'malloc' in the
 *     replay thread. For updates and deletions, any existing off-heap LN is
 *     freed here, avoiding an expensive 'free' in the replay thread.</li>
 *
 *     <li>If the DB has any triggers (which are used for some KVS metadata
 *     DBs), the LN is fetched (or loaded from off-heap cache) here to avoid
 *     this overhead in the replay thread.</li>
 * </ul>
 */
public class ReplayPreprocessor implements Runnable {
    ...
    /**
     * Returns zero or a non-zero ID for the LN to be cached off-heap.
     * When non-zero is returned, the caller becomes the owner of the
     * ID and must place it in the BIN or free it.
     */
    synchronized long takeOffHeapLnId() {
        assertState(done);
        assertState(!closed);

        final long id = offHeapLnId;
        offHeapLnId = 0;
        return id;
    }
     ...
     * execution and not used by replay. This includes unpinning the BIN and
     * freeing the off-heap LN ID.
     *
    ...
    public synchronized void close() {
        ...
        if (offHeapLnId != 0) {
            envImpl.getOffHeapCache().freeMemory(offHeapLnId);
        }
    }
        ...
        try {
            ...
            if (isInsertion) {
                ...
                if (idx >= 0) {
                    ...
                    /*
                     * Add old LN to main cache before freeing its
                     * off-heap LN to avoid a fetch from disk during the
                     * replay put. The old data is needed for triggers but
                     * not for secondaries, since secondary LNs are
                     * currently replicated independently.
                     */
                    if (hasSeparateLN && dbImpl.hasUserTriggers()) {
                        bin.fetchLN(idx, cacheMode);
                    }
                    if (bin.getOffHeapLNId(idx) != 0) {
                        envImpl.getOffHeapCache().freeLN(bin, idx);
                    }
                }
            }
            ...
            if (CursorImpl.shouldStoreOffheapLNAfterWrite(
                dbImpl, lnEntry.getLN(), isInsertion, cacheMode)) {

                offHeapLnId = envImpl.getOffHeapCache().serializeLN(
                    envImpl, vlsn, lnEntry.getModificationTime(),
                    lnEntry.getLN());
            }
        } catch (Throwable e) {
            close();
