...
public class FeederFilterChangeResult implements Serializable {
    ...
    public String toString() {
        if (status.equals(Status.FAIL)) {
            return "fail to apply filter change with id " + reqId +
                   ((error == null || error.isEmpty()) ? "" :
                       ", error " + error);
        }

        if (status.equals(Status.NOT_APPLICABLE)) {
            return "not applicable filter change with id " + reqId;
        }

        if (status.equals(Status.OK)) {
            return "filter change with id " + reqId + " applied to feeder at " +
                   "VLSN " + vlsn + ", time " +
                   new TracerFormatter().getDate(timestamp);
        }
