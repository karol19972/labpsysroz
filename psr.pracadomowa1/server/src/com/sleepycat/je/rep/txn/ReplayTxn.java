...
public class ReplayTxn extends Txn {
    ...
    /*
     * The time on the master when this transaction was committed.
     */
    private Timestamp masterCommitTime;
     ...
     */
    private long validateDTVLSN(long txnVLSN,
                                long checkDTVLSN) {

        if ((txnVLSN != INVALID_VLSN) && VLSN.isNull(checkDTVLSN)) {
            throw new IllegalStateException("DTVLSN(" + txnVLSN +") is null");
        }

        if (txnVLSN == INVALID_VLSN) {
            /*
    ...
    public void setMasterCommitTime(Timestamp masterCommitTime) {
