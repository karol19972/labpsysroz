...
final public class Feeder {
    ...
    private class InputThread extends StoppableThread {
        ...
        private void deemAcked(long txnId) {
            ...
            if (mtxn != null) {
                long ackAwaitStartNs = mtxn.getAckAwaitStartNs();
                if (ackAwaitStartNs > 0) {
                    long nowNs = System.nanoTime();
                    replicaAckLatency.add(nowNs - ackAwaitStartNs);
                }
            }
        }
    ...
    private class LogThread extends OutputThread {
        ...
        public void run() {
            ...
            } catch (IOException | MasterSyncException |
        ...
        private void writeAvailableEntries()
            ...
            do {
                OutputWireRecord record =
                    feederSource.getWireRecord(feederVLSN, waitNs);
        ...
        public void run() {
            ...
            try {
                ...
                LoggerUtils.severe(logger, repImpl,
                    ...
                    e.getMessage() + LoggerUtils.getStackTrace(e));
                throw e;
            } catch (Error e) {
                heartbeatError = e;
                repNode.getRepImpl().invalidate(e);
