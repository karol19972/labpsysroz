...
public class SubscriptionThread extends StoppableThread {
    ...
    private volatile ConcurrentMap<String, ChangeResultHandler> handlerMap;
    ...
    public void run() {
        ...
        LoggerUtils.info(logger, repImpl,
                         lm("Start subscription from VLSN " + reqVLSN +
                            " from feeder at " + config.getFeederHost() + ":" +
        ...
        try {
            ...
            LoggerUtils.warning(logger, repImpl,
                                lm("Subscription thread exited due to " +
                                   "security check failure message: " +
                                   ure.getMessage()));
            ...
            if (messageProcThread.isAlive()) {
                try {
                    ...
                    LoggerUtils.logMsg(
                        ...
                        () -> lm("exception in shutting down msg proc " +
            ...
            storedException = e;
            LoggerUtils.info(logger, repImpl,
                             lm("received group shutdown " + e.getMessage() +
                                "\n" + LoggerUtils.getStackTrace(e)));
            ...
            storedException = e;
            LoggerUtils.warning(logger, repImpl,
                                lm("unable to sync up with feeder due to EFE " +
                                   e.getMessage()));
            LoggerUtils.logMsg(logger, repImpl, Level.FINE,
                               () -> LoggerUtils.getStackTrace(e));
            status = SubscriptionStatus.UNKNOWN_ERROR;
            ...
            storedException = e;
            LoggerUtils.warning(logger, repImpl,
                                lm("interrupted exception " + e.getMessage()));
            LoggerUtils.logMsg(logger, repImpl, Level.FINE,
                               () -> LoggerUtils.getStackTrace(e));
            status = SubscriptionStatus.UNKNOWN_ERROR;
            ...
            storedException = e;
            LoggerUtils.warning(logger, repImpl,
                                lm("internal exception " + e.getMessage()));
            LoggerUtils.logMsg(logger, envImpl, Level.FINE,
                               () -> LoggerUtils.getStackTrace(e));
            status = SubscriptionStatus.UNKNOWN_ERROR;
        } catch (RuntimeException exp) {
            LoggerUtils.warning(logger, repImpl,
                                lm("unexpected exception " + exp.getMessage() +
                                   LoggerUtils.getStackTrace(exp)));
            status = SubscriptionStatus.UNKNOWN_ERROR;
        } catch (Error err) {
            LoggerUtils.warning(logger, repImpl,
                                lm("subscription thread error " +
                                   err.getMessage() + "\n" +
    ...
    synchronized void changeFilter(FeederFilterChange change,
        ...
        if (handlerMap.containsKey(change.getReqId())) {
            ...
            throw new IllegalArgumentException("A change result handler with " +
                                               "request id " +
                                               change.getReqId() + " is " +
                                               "already registered.");
        }
    ...
    void shutdown() {
        ...
        LoggerUtils.logMsg(logger, repImpl, Level.FINE,
                           () -> lm("Channel to " + config.getFeederHost() +
                                    " shut down."));
    }
    ...
    private void openChannel() throws ConnectionException,
        ...
        try {
            ...
            //TODO: throw IE to let client retry, we need better response
            // code to tell if the failure is caused by security check in
            // which case the ReplicationSecurityException should be thrown
            // instead of IE. [#27504]
    ...
    private void loopInternal() throws InternalException,
        ...
        try {
                ...
                if (messageOp.getMessageClass() == Protocol.HEARTBEAT_CLASS) {
                    ...
                    if (result.getStatus().equals(
                        ...
                        final String err =
                            ...
                            " with req id " + result.getReqId() +
                            " at " + namedChannel.getNameIdPair() +
                            ", result " + result;
                        LoggerUtils.warning(logger, envImpl, lm(err));
                        throw new FilterChangeException(result.getReqId(), err);
                    }
                    ...
                    LoggerUtils.info(logger, envImpl,
                                     lm("Receive shutdown request from feeder" +
                                        " " + config.getFeederHost() +
                                        ", shutdown subscriber"));
             ...
             */
            LoggerUtils.info(logger, envImpl,
                             lm("Connection to " + config.getFeederHost() +
    ...
    private void shutDownAuxThreads() {
        ...
        if (messageProcThread != null) {
            try {
                ...
                LoggerUtils.warning(logger, repImpl,
                                    lm("error in shutdown msg proc thread: " +
    ...
    private String lm(String msg) {
        return "[Subscription-" + config.getSubNodeName() + "] " + msg;
    }
}
