...
class DirtyINMap {
    ...
    synchronized void coordinatePruningWithCheckpoint(final IN branchParent) {
        ...
        if (ckptState != CkptState.NONE) {
            ...
            selectUnconditionally(branchParent, -1, updateMemoryBudget);
        }
    }
    ...
    private synchronized void selectForCheckpoint(final IN in,
        ...
        selectUnconditionally(in, index, false /*updateMemoryBudget*/);
    }
     ...
     */
    private synchronized void selectUnconditionally(
        final IN in,
        final int index,
        final boolean updateMemoryBudget) {
        ...
        if (db.isTemporary()) {
            /* Temp DBs are not flushed by checkpoint/sync. */
            return;
        }

        addIN(in, index, true /*updateFlushLevels*/, updateMemoryBudget);
    }
