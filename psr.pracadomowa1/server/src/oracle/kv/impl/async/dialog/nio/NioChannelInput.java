...
class NioChannelInput implements ChannelInput {
    ...
    private final String createChnlSliceDescription = String.format(
        "create slice from pool for channel input %x",
        System.identityHashCode(this));
    private final String forkReadBytesSliceDescription = String.format(
        "fork slice to read bytes for channel input %x",
        System.identityHashCode(this));
    ...
    NioChannelInput(IOBufferPool pool) {
        ...
        this.chnlSlice0 =
            IOBufSliceImpl.InputPoolBufSlice.createFromPool(
                pool, createChnlSliceDescription);
    ...
    public BytesInput readBytes(int len) {
        ...
        while (true) {
            ...
            if (chunkLen != 0) {
                bufs.add(protoSlice.forkAndAdvance(
                    chunkLen, forkReadBytesSliceDescription));
                n -= chunkLen;
            }
            if (n == 0) {
                break;
