 ...
 * based on the dialog layer. Unlike RMI, the implementation of asynchronous
 * call initiation, response, and serialization and deserialization of requests
 * and responses requires hand coding at the interface and method level.
 *
 * <p>The remote call interface is represented by a service interface that
 * extends the {@link AsyncVersionedRemote} interface and whose methods
 * represent the remote calls. The parameters of each remote method should
 ...
 * An {@code enum} that implements {@link MethodOp} provides unique IDs for
 * each remote method. Each method is represented by a {@link MethodCall} that
 * implements the marshaling and unmarshaling of requests and responses for
 * that method.
 *
 * <p>The API provided to callers should be implemented as a subclass of {@link
 * AsyncVersionedRemoteAPI}, which negotiates the serial version to support
 * upgrades. The API implementation should use a subclass of {@link
 * AsyncVersionedRemoteInitiator} that implements the remote API and that is
 * used to initiate remote calls.
 *
