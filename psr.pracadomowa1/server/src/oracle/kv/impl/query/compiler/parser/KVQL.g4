...
cond_expr : comp_expr | in_expr | exists_expr | is_of_type_expr ;
...
insert_statement :
    prolog?
    (INSERT | UPSERT) INTO table_name (AS? tab_alias)? (LP id (COMMA id)* RP)?
...
column_def : id type_def (default_def | identity_def | uuid_def)? comment? ;
...
alter_def : alter_field_statements | ttl_def | add_region_def | drop_region_def;
...
add_field_statement : ADD schema_path type_def (default_def | identity_def | uuid_def)?
    comment? ;
...
create_index_statement :
    ...
    ((LP index_path_list RP (WITH NO? NULLS)?) |
...
id :
    (ACCOUNT | ADD | ADMIN | ALL | ALTER | ALWAYS| ANCESTORS | AND | ANY_T |
     ...
     BY | CACHE | CASE | CAST | COMMENT | COUNT | CREATE | CYCLE |
     DAYS | DECLARE | DEFAULT | DELETE | DESC | DESCENDANTS | DESCRIBE |
     DISTINCT | DROP |
     ELEMENTOF | ELSE | END | ES_SHARDS | ES_REPLICAS | EXISTS | EXTRACT |
     FIRST | FROM | FULLTEXT | GENERATED | GRANT | GROUP | HOURS |
     IDENTIFIED | IDENTITY | IF | INCREMENT | INDEX | INDEXES | INSERT | INTO |
     IN | IS | JSON | KEY | KEYOF | KEYS |
     LIFETIME | LAST | LIMIT | LOCAL | LOCK | MINUTES | MODIFY |
     NAMESPACE | NAMESPACES | NESTED | NO | NOT | NULLS |
     OF | OFFSET | ON | OR | ORDER | OVERRIDE |
     PASSWORD | PRIMARY | PUT | RDIV | REGION | REGIONS | REMOVE | RETURNING |
     ROLE | ROLES | REVOKE |
     SECONDS | SELECT | SEQ_TRANSFORM| SET | SHARD | SHOW | START |
     TABLE | TABLES | THEN | TO | TTL | TYPE |
     UNLOCK | UPDATE | UPSERT | USER | USERS | USING | VALUES | WHEN | WHERE | WITH |
