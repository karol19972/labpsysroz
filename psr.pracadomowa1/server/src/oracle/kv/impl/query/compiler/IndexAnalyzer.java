...
class IndexAnalyzer implements Comparable<IndexAnalyzer> {
    ...
    final static int filterEqValue = 25;
    ...
    public int compareTo(IndexAnalyzer other) {
        ...
        if (theIsCovering != other.theIsCovering) {
            ...
            if (other.theIsCovering) {
                ...
                if (theTrace >= 2) {
                    System.out.println("Index " + other.getIndexName() +
                                       " is covering");
                }
    ...
    void apply(IndexAnalyzer primaryAnalyzer) {
        ...
        //System.out.println("Selected index: " + getIndexName());
    ...
    private Expr rewriteINPred(ExprVar idxVar, PredInfo inPi) {
        ...
        inExpr.setNumKeyComps(inExpr.getNumArgs());
    ...
    void collectPartitionShardPred(
        ...
        if (constVal != null) {
            ...
            if (isShard) {
                ...
                numContainers = FieldDefImpl.integerDef.
    ...
    private void collectPredInfosForInOp(WherePredInfo wpi, Expr pred) {
        ...
        for (int i = 0; i < numKeyComps; ++i) {
            ...
            if (epath.getFilteringPreds() != null || epath.theIsUnnested) {
                // TODO ????
                pi.theStatus = PredicateStatus.SKIP;
                continue;
            }
            ...
            if (epath.theIsMultiKey && pi.theVarArg.isMultiValued()) {
                pi.theStatus = PredicateStatus.SKIP;
                continue;
            }
    ...
    private boolean checkTypes(Expr varArg, Expr constArg, boolean isIn) {
        ...
        if (epath != null && epath.getJsonDeclaredType() != null) {
            ...
            if (!varType.isPrecise()) {
                ...
                if (!constType.isSubtype(FieldDefImpl.anyJsonAtomicDef)) {
                    return false;
                }
                return true;
            }
                ...
                for (int i = 0; i < numKeys; ++i) {
                    ...
                    for (int k = 0; k < numInSearchKeys; ++k) {
                        ...
                        for (PredInfo pi : inPIs) {
                            ...
                            if (ipath2.isMultiKey() && !theHaveMapKeyEqPred) {
    ...
    private void checkIsCovering() {
         ...
         */
        boolean isKeyOnly = theTable.isKeyOnly();
        ...
        if (!theIsCovering) {
            ...
            if (theTrace >= 2) {
                System.out.println(
                    "Index " + getIndexName() + " is not covering: it does " +
                    "not cover all predicates");
            }
            return;
        }
        ...
        if (theTrace >= 2) {
            System.out.println(
                "Index " + getIndexName() + " is covering = " + theIsCovering);
        }
    }
    ...
    private void checkIsSortingIndex() {
        ...
        if (theIsPrimary) {
            ...
            for (i = 0; i < numPkCols && i < numExprs; ++i) {
                ...
                if (!ExprUtils.isPrimKeyColumnRef(theTable, i, expr)) {
                    break;
                }
        ...
        for (i = 0, k = 0; i < indexPaths.size() && k < numExprs; ++i) {
            ...
            if (iexpr.matchesIndex(theIndex, ipath.getPosition())) {
                ++k;
                continue;
            }
        ...
        if (i == indexPaths.size()) {
            ...
            for (int j = 0;
                ...
                if (!ExprUtils.isPrimKeyColumnRef(theTable, j, expr)) {
                    break;
                }
            }
