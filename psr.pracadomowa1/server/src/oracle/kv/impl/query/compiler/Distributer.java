...
class Distributer extends ExprVisitor {
    ...
    void exit(ExprDeleteRow del) {
        ...
        if (!del.hasReturningClause() &&
            dkind != DistributionKind.SINGLE_PARTITION) {
    ...
    boolean enter(ExprSFW sfw) {
        ...
        theQCB.setHasGroupByExpr(sfw.getNumGroupExprs() > 0);
         ...
         */
        if (isSinglePartition && !eliminateIndexDups) {
            return false;
        }
