...
import oracle.kv.shaded.org.antlr.v4.runtime.ParserRuleContext;
import oracle.kv.shaded.org.antlr.v4.runtime.tree.ParseTree;
import oracle.kv.shaded.org.antlr.v4.runtime.tree.ParseTreeWalker;
import oracle.kv.shaded.org.antlr.v4.runtime.tree.RuleNode;
import oracle.kv.shaded.org.antlr.v4.runtime.tree.TerminalNode;
...
//            }
    ...
    /*
     * from_clause :
     *    FROM (from_table | nested_tables | left_outer_join_tables)
    ...
    private void translateTable(
        ...
        if (isRightTable) {
            ...
            if (theTables.size() > 1) {
                 ...
                 *           LOJ A.B.C.D.E.F f
                 */
                TableImpl last = theTables.get(theTables.size() - 1);
    ...
    private void traverseTables(
        ...
        if (index == 0) {
            ...
            if (orExpr != null) {
                ...
                pred = theExprs.pop();
                pred = validateOnExpr(tables, aliases, targetTable, table,
                                      index, pred);
            }
        }
    ...
    private Expr validateOnExpr(
        ...
        if (top.getFuncCode() == FuncCode.OP_EQ) {
            if (isPrimaryKeyEQExpr(left, table, 0, top)) {
                foundPkCols[0] = true;
            }
        } else  {
            /* traverses the expression tree, find and remove join predicates. */
            findAndRemoveJoinPreds(left, table, numLojPKCols, foundPkCols, top);
        }
    ...
    private void findAndRemoveJoinPreds(
        ...
        while (iter.hasNext()) {
            ...
            if (exp.getFunction(FuncCode.OP_EQ) != null) {
                ...
                for (int pkCol = 0; pkCol < numLojPkCols; pkCol++) {
                    if (isPrimaryKeyEQExpr(left, right, pkCol, fc)) {
                        joinPreds.add(fc);
                        retPkPos[pkCol] = true;
                        break;
                    }
    ...
    private boolean isPrimaryKeyEQExpr(
        ...
        final ExprIter iter = expr.getChildrenIter();
        while (iter.hasNext()) {
            Expr e = iter.next();
            if (!foundLeft) {
                foundLeft = ExprUtils.isPrimKeyColumnRef(left, pkCol, e);
                if (foundLeft) {
                    continue;
                }
            }
            if (!foundRight) {
                foundRight = ExprUtils.isPrimKeyColumnRef(right, pkCol, e);
            }
        }
        iter.reset();
    ...
    private void checkInvalidColumnRef(Expr top, TableImpl table) {
        ...
        final ExprIter iter = top.getChildrenIter();
    ...
    public void exitWhere_clause(KVQLParser.Where_clauseContext ctx) {
        ...
        if (near != null) {
            ...
            /*
             * Let the QCB know that geo_near is used, as it requires client
             * side sorting.
             */
            theQCB.setGeoNear(true);
        }
    }
    ...
    public void exitLimit_clause(
        ...
        theQCB.setHasOffsetOrLimit(true);
    }
     ...
     *
     * cond_expr := comp_expr | exists_expr | is_of_type_expr | in_expr;
     */
    @Override
    ...
    public void exitFunc_call(KVQLParser.Func_callContext ctx) {
        ...
        Expr expr = ExprFuncCall.
                    create(theQCB, theInitSctx, loc, func, inputs);
    ...
    /**
     * column_def : id type_def (default_def | identity_def)? comment? ;
     */
    @Override
     ...
     *     LP index_path_list RP [WITH [NO] NULLS] comment?;
     *
     * index_name : id ;
     *
    ...
    public void enterCreate_index_statement(
        ...
        List<KVQLParser.Index_pathContext> pathCtxs =
        ...
        theQCB.getStatementFactory().createIndex(namespace,
            ...
            typeArray, indexNulls, null /* annotatedFields */, geoProps,
