...
public class FuncTrimIter extends PlanIter {
    ...
    public boolean next(RuntimeControlBlock rcb) {
        ...
        if (theArgs.length >= 3) {
            ...
            String charParam = argValue.asString().get();
            if (charParam.length() < 1) {
                rcb.setRegVal(theResultReg,
                              FieldDefImpl.stringDef.createString(str));
                state.done();
                return true;
            }
        ...
        FieldValueImpl res = FieldDefImpl.stringDef.createString(resStr);
