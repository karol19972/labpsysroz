...
public class PartitionUnionIter extends PlanIter {
    ...
    public boolean sortingNext(
        ...
        if (!initRI.isInSortPhase1()) {
            ...
            /*
             * If the ResumeInfo.theGBTuple is not null, there remains last
             * aggregation result in theInputIter. In this case, don't mark the
             * state as DONE, then read last result in next call.
             */
            if (!rcb.getReachedLimit() &&
                initRI.getPrimResumeKey() == null &&
                initRI.getGBTuple() == null) {
                state.done();
            }
        ...
        while(true) {
            ...
            if (more) {
                ...
                /*
                 * Handle grouping query: the resume key may be null (meaning
                 * there are no more results from this partition), even though
                 * a result is returned
                 */
                if (ri.getPrimResumeKey() == null) {
                    /*
                     * If ResumeInfo.theGBTuple is not NULL, add the remaining
                     * aggregation result to results.
                     */
            ...
            if (rcb.getReachedLimit() ||
                (batchSize > 0 && state.theNumResults > batchSize)) {
    ...
    public boolean simpleNext(
        ...
        while(true) {
             ...
             * partition, mark the current partition as done and move to
             * the next one.
             */
            if (ri.getPrimResumeKey() == null) {
                moveToNextPartition(rcb, state);
