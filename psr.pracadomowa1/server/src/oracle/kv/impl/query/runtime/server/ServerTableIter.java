...
public class ServerTableIter extends BaseTableIter {
    ...
    final private HashSet<BinaryValueImpl> thePrimKeysSet;
    ...
    private void computeIndexScans(RuntimeControlBlock rcb) {
        ...
        for (int i = 0; i < theExternalKeysIters.length; ++i) {
            ...
            if (in3Iters != null && in3Iters[i] >= 0) {
                ArrayList<FieldValueImpl> bindvals =
        ...
        for (int field = 0; field < numFields; ++field) {
            ...
            for (int scan = 0; scan < numScans; ++scan) {
                ...
                if (in3bi != null &&
                    ...
                    if (!val.isNull()) {
                        val = castBindVal(table, index, field, val, FuncCode.OP_EQ);
                    }
                    ...
                    if (bindMap[field] >= 0) {
                        ...
                        val = castBindVal(table, index, field, val, FuncCode.OP_GE);
                        bindVals[bindMap[field]] = val;
                    ...
                    if (bindMap[field+1] >= 0) {
                        ...
                        val = castBindVal(table, index, field, val, FuncCode.OP_LE);
   ...
   private int findTablePos(TableImpl table) {
    ...
    public boolean simpleNext(RuntimeControlBlock rcb, PlanIterState state) {
        ...
        byte[] primResumeKey = null;
        byte[] secResumeKey = null;
        ResumeInfo ri = rcb.getResumeInfo();

        if (state.isOpen()) {
            state.setState(StateEnum.RUNNING);
            primResumeKey = ri.getPrimResumeKey();
            secResumeKey = ri.getSecResumeKey();
            if (theFirstCall < 0) {
                theFirstCall = 1;
            }
        }
        ...
        try {
            ...
            while (more) {
                ...
                if (filterIter != null) {
                    ...
                    /* Don't get the row size if the query is using a covering
                     * secondary index, because doing so will access the LN */
                    if (theIndex == null || !theUsesCoveringIndex[0]) {
                    ...
                    if (!match) {
                        if (rcb.getTraceLevel() >= 2) {
                            rcb.trace("Filtered-out index row :\n " + indexRow);
                        }
                ...
                if (theUsesCoveringIndex[0]) {
                    if (theLockIndexEntries || theScanner.lockIndexRow()) {
                        ...
                        if (rcb.getTraceLevel() >= 3) {
                            rcb.trace("ServerTableIter: row storage size = " +
                                      tv.getStorageSize());
                            rcb.trace("ServerTableIter: index storage size = " +
                                      tv.getIndexStorageSize());
                            rcb.trace("ServerTableIter: partition id = " +
                                      tv.getPartition());
                        }
                    ...
                    if (tableRow != null) {
                        ...
                        rcb.trace("Failed to access table row for" +
                                  " index row");
                    }
                }
            ...
            if (rcb.getTraceLevel() >= 2) {
                rcb.trace("SFW: SizeLimitException. firstCall = " +
                          theFirstCall +
                          " MaxReadKB = " + rcb.getMaxReadKB() +
                          " CurrentMaxReadKB = " + rcb.getCurrentMaxReadKB());
            }

            if (theFirstCall == 1 &&
                rcb.getMaxReadKB() == rcb.getCurrentMaxReadKB() &&
                Arrays.equals(ri.getPrimResumeKey(), primResumeKey) &&
                Arrays.equals(ri.getSecResumeKey(), secResumeKey)) {
                throw new QueryException(
    ...
    private boolean resume(RuntimeControlBlock rcb)
        ...
        if (joinPathKey == null) {
            ...
            byte[] descResumeKey = ri.getDescResumeKey();

            try {
                more = theScanner.next(theTables[theTargetTablePos]);
            } catch (SizeLimitException e) {

                if (!Arrays.equals(primResumeKey, ri.getPrimResumeKey()) ||
                    !Arrays.equals(descResumeKey, ri.getDescResumeKey())) {
                    theInResume = false;
                }
                throw e;
            }
