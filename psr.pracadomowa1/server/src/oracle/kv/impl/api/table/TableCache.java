...
class TableCache extends LruCache<String, TableImpl> {
    ...
    static {
        assert KVVersion.PREREQUISITE_VERSION.
            compareTo(SerialVersion.getKVVersion(TABLE_SEQ_NUM_VERSION)) < 0 :
            "Workaround for pre-table seq number servers" +
            " can be removed";
    }
    /*
    ...
    TableCache(int initialCapacity, long lifetimeMS) {
        /*
         * Set the lifetime to 0 in the constructor so that the cleanup thread
         * is not started. Set it to desired value after return.
         */
    ...
    TableImpl put(TableImpl table) {
        ...
        try {
            ...
            /*
             * If the sequence number of the table is 0, it is either an old
             * table (pre-upgrade) or an old server. In this case set the
             * sequence number of the cached entry to the highest seq number
             * seen. This will keep the entry in the cache when talking to
             * an old server (which is returning the metadata sequence number
             * in the Result). Though not needed in the new server case, it
             * does no harm.
             * This can be removed once the pre-req >= TABLE_SEQ_NUM_VERSION
             */
            if (top.getSequenceNumber() == 0) {
                top.setSequenceNumber(highSeqNum.get());
            }
