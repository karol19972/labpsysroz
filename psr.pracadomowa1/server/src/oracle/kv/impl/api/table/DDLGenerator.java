...
public class DDLGenerator {
     ...
     */
    private void generateDDL(boolean withIfNotExists) {
        ...
        if (tableImpl.isMultiRegion()) {
            ...
            for (int regionId : tableImpl.getRemoteRegions()) {
                tableSb.append(", ");
                final String regionName = regionMapper.getRegionName(regionId);
                if (regionName == null) {
                    throw new RuntimeException("Unable to create DDL," +
                                               " unknown region ID " + regionId);
                }
    ...
    private String getIndexDDL(Index index, boolean withIfNotExists) {
        ...
        for (int i = 0; i < numFields; i++) {
            ...
            if (type != null) {
                sb.append(" as " + IndexField.getDDLTypeString(type));
            }
