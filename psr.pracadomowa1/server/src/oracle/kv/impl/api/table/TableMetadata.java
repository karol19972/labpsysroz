...
public class TableMetadata implements TableMetadataHelper,
    ...
    public final void initializeTables(TableMetadata md) {
        assert tables == null;
        tables = new TreeMap<>(FieldComparator.instance);
        if (md != null) {
    ...
    public boolean evolveTable(TableImpl table, int tableVersion,
        ...
        if (fieldMap.equals(table.getFieldMap()) &&
            ...
            compareRegions(table.getRemoteRegions(), regions)) {
            return false;
        }
     ...
     */
    private TableImpl filterTable(TableImpl table) {
    ...
    private TableList getSystemTables() {
        final TableList list = new TableList(seqNum);
    ...
    private void ensureNamespaceMap() {
        if (namespaces == null) {
            namespaces = new TreeMap<>(FieldComparator.instance);
            /*
     ...
     *
     * @return true if the metadata was changed
     */
    public boolean setLocalRegionName(String localName) {
        if ((localName == null) || localName.isEmpty()) {
            throw new IllegalCommandException("Bad region name: " + localName);
        }
        ...
        if (knownRegions == null) {
            ...
            addActiveRegion(new Region(localName, Region.LOCAL_REGION_ID));
            return true;
        }
    ...
    /**
     * Creates the specified region. Returns true if the metadata was modified.
     */
    public boolean createRegion(String regionName) {
        ...
        addActiveRegion(new Region(regionName, knownRegions.size()));
        return true;
    }
    ...
    private void addActiveRegion(Region region) {
        if (activeRegions == null) {
            activeRegions = new TreeMap<>(FieldComparator.instance);
        }
    ...
    /**
     * Returns the names of all active region names.
     */
    public Set<String> getActiveRegions() {
        return activeRegions == null ? Collections.emptySet() :
                                       activeRegions.keySet();
    }
    ...
    public static class TableList extends InfoList<Table> {
        ...
        TableList(int seqNum, Collection<Table> tables) {
            super(seqNum, tables);
        }
     ...
     */
    private static class RegionMapperImpl implements RegionMapper,
                                                     MetadataInfo,
                                                     Serializable {
        private static final long serialVersionUID = 1L;
