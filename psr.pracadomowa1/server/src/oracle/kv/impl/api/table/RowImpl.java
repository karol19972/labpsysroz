...
public class RowImpl extends RecordValueImpl implements Row, RowSerializer {
    ...
    void checkIdentity(int pos, FieldValue value) {
        ...
        if (table != null &&
            ...
            throw new IllegalArgumentException("Cannot set value for a " +
     ...
     *
     * @throws UnsupportedOperationException if invoked on a table that is
     * not multi-region table
     */
    @Override
    public int getRegionId() {
        if (!isFromMRTable()) {
            throw new UnsupportedOperationException("Region ID is not " +
