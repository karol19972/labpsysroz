...
/**
 * An {@link Encoder} for binary encoding.
 * <p/>
 ...
 * <p/>
 * To construct, use
 * {@link EncoderFactory#binaryEncoder(OutputStream, byte[])}.
 * <p/>
...
public class BufferedBinaryEncoder extends BinaryEncoder {
     ...
     */
    BufferedBinaryEncoder(OutputStream out, int bufferSize, byte[] buffer) {
        if (null == out)
     ...
     * of BinaryEncoder.
     * <p/>
     * Currently the only destination option is an OutputStream, but we may
     * later want to handle other constructs or specialize for certain
     * OutputStream Implementations such as ByteBufferOutputStream.
     * <p/>
     */
    private abstract static class ByteSink {
        ...
        protected abstract void innerWrite(byte[] bytes, int off, int len)
                throws IOException;
