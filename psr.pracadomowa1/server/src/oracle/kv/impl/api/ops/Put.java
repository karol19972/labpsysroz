...
public class Put extends SingleKeyOperation {
    ...
    public byte[] getValueBytes() {
        boolean isMultiRegionTable =
            (requestValue.getValueFormat() == Format.MULTI_REGION_TABLE);
        if (isMultiRegionTable && requestValue.getRegionId() !=
                                  Region.LOCAL_REGION_ID) {
            throw new FaultException("The region id must be one " +
                "for local write: " + requestValue.getRegionId(), true);
        }
