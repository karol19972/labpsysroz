...
import static oracle.kv.impl.admin.param.StorageNodeParams.MAX_COMPRESSED_OOPS_HEAP_SIZE;
...
public class RepEnvHandleManager implements ParameterListener {
    ...
    private EnvironmentConfig createEnvConfig(final ParameterUtils pu) {
        ...
        final long rnMaxOffHeap = rnParams.getRNMaxOffHeap(snParams);
        config.setOffHeapCacheSize(rnMaxOffHeap);
     ...
     * Note that heap size smaller than 32GB limit, or a value of 100% for
     * SN_SYSTEM_PERCENT would disable the offheap cache, which should be
     * handled specially.
     *
     * A negative value for offheap cache size indicates that, current SN
     * memory allocation violates the rule of total memory use <= SN memory.
     */
    private void checkMemoryAllocation(Params params) {

        final StorageNodeParams snp = params.getStorageNodeParams();
        final long snMemorySize = snp.getMemoryMB();
        if (snMemorySize == 0) {
        ...
        final long totalHeapSize = (rnp.getMaxHeapBytes() / (1024 * 1024)) *
            ((snp.getCapacity() != 0) ? snp.getCapacity() : 1);

        final int jvmOverheadPercent = snp.getJVMOverheadPercent();

        /*
         * If heap size doesn't hit 32GB limit, or systemPercent=100, offheap
         * cache is disabled, and system uses all of the available memory for
         * the heap.
         */
        final boolean disableOffheapCache;
        if(!RepNodeParams.EXPLICITLY_ENABLE_OFFHEAP_CACHE) {
            disableOffheapCache =
                ((rnp.getMaxHeapBytes() < MAX_COMPRESSED_OOPS_HEAP_SIZE) ||
                 (snp.getSystemPercent() == 100));
        } else {
            disableOffheapCache = (snp.getSystemPercent() == 100);
        }

        final long totalSystemSize = disableOffheapCache ?
            snMemorySize - (totalHeapSize * (jvmOverheadPercent + 100) / 100) :
            (snMemorySize * snp.getSystemPercent()) / 100;

        final long residualSize =
            snMemorySize -
            (totalHeapSize * (jvmOverheadPercent + 100) / 100) -
            totalSystemSize;

        final long offheapSizePerRN =
            residualSize / ((snp.getCapacity() != 0) ? snp.getCapacity() : 1);

        /* check SN memory allocation, and make INFO or WARNING logging */
        final int totalJavaMemoryPercent = snp.getRNHeapPercent() +
            ((snp.getRNHeapPercent() * jvmOverheadPercent) / 100);
        final int totalRequestedPercent = disableOffheapCache ?
            100 :
            totalJavaMemoryPercent + snp.getSystemPercent();
        ...
        final String memoryStats =
            String.format(" rnHeapPercent:%d" +
                          ...
                          " systemPercent:%d" +
                          " capacity:%d" +
                          " Total Java memory percent:%d" +
                          " Total requested percent:%d" +
                          " Off-heap cache percent:%d" +
                          " SN memory size:%,d MB" +
                          " Java heap size:%,d MB" +
                          " JVM overhead size:%,d MB" +
                          " System memory size:%,d MB" +
                          " Off-heap cache size:%,d MB" +
                          " Heap size per RN:%,d MB" +
                          " Off-heap cache size per RN:%,d MB",
                          snp.getRNHeapPercent(),
                          jvmOverheadPercent,
                          snp.getSystemPercent(),
                          snp.getCapacity(),
                          totalJavaMemoryPercent,
                          totalRequestedPercent,
                          100 - totalRequestedPercent,
                          snMemorySize,
                          totalHeapSize,
                          (totalHeapSize * jvmOverheadPercent) / 100,
                          totalSystemSize,
                          residualSize,
                          rnp.getMaxHeapBytes() / (1024 * 1024),
                          offheapSizePerRN);

        if (checkMemoryAllocation(totalJavaMemoryPercent,totalRequestedPercent,
                                  totalSystemSize,residualSize)) {
            logger.info("SN memory allocation obeys allocation rules:" +
                        memoryStats);
        } else {
            logger.warning(
                "SN memory allocation violates allocation rules. " +
                "There is a shortfall in SN memory size, " +
                "or total memory percentage exceeds 100%:" +
                memoryStats);
        }

    }

    public static boolean checkMemoryAllocation(int totalJavaMemoryPercent,
                                                int totalRequestedPercent,
                                                long totalSystemSize,
                                                long residualSize) {

        /* total memory percentage exceeds 100% */
        if (totalJavaMemoryPercent >=100 || totalRequestedPercent > 100)
        {
            return false;
        }

        /*
         * SN memory allocation should reserve SN memory space for heap,
         * JVM overhead, and system use(non-zero).
         */
        if ((residualSize >= 0) && (totalSystemSize > 0)) {
            return true;
        }
        return false;
    }
    ...
    private void networkRestore(InsufficientLogException ile) {
        ...
        try {
            ...
            repNodeService.getFaultHandler().
                queueShutdown(efe, ProcessExitCode.RESTART);
