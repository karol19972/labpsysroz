...
public class MasterBalanceStateTracker extends StateTracker {
    ...
    protected void doNotify(StateChangeEvent sce) throws InterruptedException {
        ...
        while (!skipRetry) {
            ...
            try {
                 ...
                 */
                skipRetry = handleSessionAccessExceptionForRetry();
            } catch (SNAFaultException sfe) {
                /*
                 * When remote storage node is having transient problems to
                 * validate login token, SNAFaultException will be thrown with
                 * SessionAccessException as fault class, which is expected to
                 * be resolved with retries. [KVSTORE-942]
                 */
                if (SessionAccessException.class.getName().equals(
                    sfe.getFaultClassName())) {
                    skipRetry = handleSessionAccessExceptionForRetry();
                } else {
    ...
    /*
     * Used to handle SessionAccessException, returns true
     * if expect caller to skip retry.
     */
    private boolean handleSessionAccessExceptionForRetry()
        throws InterruptedException {

        if (isShutdown()) {
            /*
             * In secure store, the remote call may fail if the server
             * side exits as part of the ShutdownThread shutdown,
        ...
        /*
         * If there is new state info in the queue,
         * skip the retry with old state info.
         */
        return !isEmpty();
    }

    /*
     * Used to handle RemoteException and SNAFaultException that
     * isn't caused by SessionAccessException, returns true
     * if expect caller to skip retry.
     */
