...
class MigrationSource implements Runnable {
    ...
    public void run() {
        ...
        final ThroughputTracker tc = repNode.getAggregateThroughputTracker();
        ...
        try {
            ...
            while (!canceled) {
                ...
                try {
                    final OperationResult result = cursor.get(key, value,
                                                              Get.NEXT,
                                                              readOptions);
                    if (result != null) {
                        /* Track the cost for load management */
                        tc.addReadBytes(getStorageSize(cursor), false);
    ...
    private void transferComplete() {
        ...
        /*
         * If transfer only mode the partition generation table does not
         * require updating.
         */
        long endVLSNSeq = 0;
        int genNum = 0;
        if (!transferOnly) {
            final PartitionGeneration pg = updatePartGenTable();
            if (pg == null) {
                /* fail to update the generation table, cancel */
                cancel(false);
                return;
            }

            endVLSNSeq = pg.getEndVLSN();
    ...
    /**
     * Updates the partition generation table before EOD is sent.
     *
     * @return the closed generation if successful, null otherwise.
     */
    private PartitionGeneration updatePartGenTable() {
        final PartitionManager pm = repNode.getPartitionManager();
        if (pm == null) {
            /*
             * We shall not reach here because the migration source is
             * created by migration service which is managed by
             * migration manager. When reaching here, the parent RN
             * is fully initialized and partition manager should be
             * ready. If not, stop the migration.
             */
            logger.warning(() -> "Unexpected error, stopping " + this +
                                 "because partition manager not initialized " +
                                 "at " + repNode.getRepNodeId());
            return null;
        }

        final PartitionGenerationTable pgt = pm.getPartGenTable();
        if (!pgt.hasPartition(partitionId)) {
            logger.warning(() -> "Unexpected error, stopping " + this +
                                 " because generation table does not have " +
                                 partitionId);
            return null;
        }

        if (pgt.getOpenGen(partitionId) == null) {
            /*
             * If reach here, that means the generation already closed when
             * EOD was last sent successfully. If the migration manager
             * resend EOD idempotently, possibly due to failure after the
             * last EOD was sent, we get the already closed generation and
             * send it with this EOD.
             */
            logger.fine(() -> partitionId + " already closed (" +
                              pgt.getLastGen(partitionId) + ") " +
                              "and will close with later VLSN. ");
        }

        try {
            pgt.closeGeneration(partitionId, pgt.getLastVLSN());
            return pgt.getLastGen(partitionId);
        } catch (Exception ex) {
            logger.info(() -> "Canceling " + this + ", exception when closing" +
    ...
    private boolean filterOp(DatabaseEntry key) {
         ...
         * be forwarded
         */
        return (lastKey != null) && (partitionDb.compareKeys(lastKey, key) < 0);
    }

    private synchronized void sendCopy(DatabaseEntry key,
                                       DatabaseEntry value,
                                       long vlsn,
                                       long expirationTime,
                                       boolean isTombstone) {
