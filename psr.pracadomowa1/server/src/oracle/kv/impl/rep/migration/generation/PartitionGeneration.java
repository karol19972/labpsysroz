...
public class PartitionGeneration implements
    ...
    private transient long prevGenVLSN = INVALID_VLSN;
    ...
    public int compareTo(PartitionGeneration o) {
        ...
        if ((value = ((Long)startVLSN).compareTo(o.getStartVLSN())) != 0) {
            return value;
        }
        if ((value = prevGenGroup.compareTo(o.getPrevGenRepGroup())) != 0) {
            return value;
        }
        return ((Long)prevGenVLSN).compareTo(o.getPrevGenEndVLSN());
    }
    ...
    public String toString() {
        return pid + ": gen # " + genNum + " vlsn[" + startVLSN + ", " +
            (endVLSN == OPEN_GEN_END_VLSN ? "Inf"
                : endVLSN) + "]" +
            (!prevGenGroup.isNull() ?
             (" from " + prevGenGroup + "(last vlsn " +
