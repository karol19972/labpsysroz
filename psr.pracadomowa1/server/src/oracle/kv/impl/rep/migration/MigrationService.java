...
public class MigrationService implements Runnable {
    ...
    private void processRequest(DataChannel channel) throws IOException {
         ...
         * an error. This will fail the migration. We do this because there is
         * currently a deadlock with partition migration plan and the store
         * version update plan. If this deadlock is resolved, this can be
         * changed to a busy response.
         */
        if (!transferOnly &&
            repNode.getStoreVersion().
                compareTo(MIN_STORE_VER_PART_GEN_DB) < 0) {
            reportError(Response.INVALID,
                        "Cannot migrate " + partitionId +
                        " because the store (ver=" + repNode.getStoreVersion() +
                        ") has not yet " +
                        "upgraded to the minimal version for partition " +
                        "generation db " + MIN_STORE_VER_PART_GEN_DB,
                        channel);
            return;
        }
