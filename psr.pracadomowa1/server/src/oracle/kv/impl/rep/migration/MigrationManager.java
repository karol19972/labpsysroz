...
public class MigrationManager implements Localizer {
    ...
    void initializeGenerationTable() {
        ...
        logger.info(() -> "Partition generation table ready");
    }
    ...
    private void openDbs(ReplicatedEnvironment repEnv,
        ...
        if (state.isReplica()) {
            if (generationDb != null) {
                /* close generation db since it is never used on replica */
        ...
        try {
            ...
            /* The generation DB only needs to be opened on the master */
            if (state.isMaster() && generationDb == null) {
                generationDb = repEnv.openDatabase(
                    txn, PartitionGenDBManager.getDBName(), dbConfig);
            } else {
                logger.fine(() -> "Skip opening generation db for non-" +
                                  "master or already opened on master," +
                                  "state:" + state.name());
            }
            ...
            if (migrationDb == null) {
                migrationDb = repEnv.openDatabase(
                    txn, PartitionMigrations.MIGRATION_DB_NAME, dbConfig);
            }
