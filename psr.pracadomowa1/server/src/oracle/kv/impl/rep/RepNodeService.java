...
import oracle.kv.impl.async.DialogResourceManager.NotePermitsAdjusted;
...
public class RepNodeService
    ...
    /**
     * If non-null, the most recently registered callback that should update
     * the thread pool parameters when the number of permits are changed in the
     * dialog layer. Synchronize on the updatedThreadPoolLock when accessing
     * this field.
     */
    private UpdateThreadPoolCallback updateThreadPoolCallback;
    private final Object updateThreadPoolLock = new Object();
    ...
    public void initialize(SecurityParams securityParams,
        ...
        requestDispatcher = RequestDispatcherImpl.createForRN
    ...
    public void newParameters(ParameterMap oldMap,
        ...
        final int oldMaxRequests = oldParams.getAsyncMaxConcurrentRequests();
        final int newMaxRequests = newParams.getAsyncMaxConcurrentRequests();

        /*
         * Changing the maximum number of requests involves changing two
         * internal parameters: the maximum number of permits for reading
         * dialogs and the maximum pool capacity. It may not be possible to
         * change the maximum number of permits immediately because of active
         * dialogs holding permits. We are required to ensure that, at any
         * given time, the actual number of permits is less than the maximum
         * pool capacity. The following mechanism meets this requirement.
         *
         * If reducing the maximum number of requests, reduce the permits first
         * and register a callback to reduce the pool capacity afterwards. If
         * increasing the number of requests, increase the pool first. The
         * changes made by this procedure meets the requirement. We further
         * make sure that when the callback for modifying the pool capacity is
         * invoked, only the last callback will be effective, and therefore,
         * this procedure meets the requirement as well.
         *
         * Note that we are changing the number of permits for the endpoint
         * group, which may not be the right thing in thread mode where the
         * endpoint group is shared across multiple services. Although kvlite
         * supports thread mode by default and allows specifying multiple SNs
         * and capacity > 1, those multi-service configurations are not
         * documented and not enabled by default. For that reason, ignore this
         * issue, at least for now, to simplify testing.
         */
        synchronized (updateThreadPoolLock) {
            if (oldMaxRequests > newMaxRequests) {
                updateThreadPoolCallback =
                    new UpdateThreadPoolCallback(oldParams, newParams);
                endpointGroup.getDialogResourceManager().setNumPermits(
                    newMaxRequests, updateThreadPoolCallback);
            } else {
                /*
                 * Clear the callback so that the next changes to the async
                 * thread pool are sure to happen instead
                 */
                updateThreadPoolCallback = null;
    ...
    /**
     * A callback that adjusts the thread pool parameters after the dialog
     * layer permits have been adjusted, but only if it is the currently
     * registered callback.
     */
    private class UpdateThreadPoolCallback implements NotePermitsAdjusted {
        private final RepNodeParams oldParams;
        private final RepNodeParams newParams;

        UpdateThreadPoolCallback(RepNodeParams oldParams,
        ...
        public void onComplete(int n) {
            synchronized (updateThreadPoolLock) {
                if (updateThreadPoolCallback == this) {
                    updateThreadPoolCallback = null;
                    reqHandler.updateAsyncThreadPoolParams(oldParams,
                                                           newParams);
                }
            }
        }
    ...
    public void stop(boolean force, boolean checkStream) {
        stopRequested = true;
