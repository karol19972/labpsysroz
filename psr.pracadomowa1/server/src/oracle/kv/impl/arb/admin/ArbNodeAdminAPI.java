...
public class ArbNodeAdminAPI extends RemoteAPI {
    ...
    public ArbNodeInfo getInfo()
        ...
        /*
         * Check if talking to old version of Arbiter that had
         * bug that could cause the getInfo() method to fail. [#27808]
         * This method is only called for the show upgrade-order command.
         * Only the software version is being used from the ArbNodeInfo. If
         * the Arbiter does not support the getInfo() method, return an
         * ArbNodeInfo with only the software version set.
         */
        final short serialVersion = getSerialVersion();
        if (serialVersion >= SerialVersion.V22) {
            return proxyRemote.getInfo(NULL_CTX, serialVersion);
        }
        return new ArbNodeInfo(SerialVersion.getKVVersion(serialVersion));
    }
