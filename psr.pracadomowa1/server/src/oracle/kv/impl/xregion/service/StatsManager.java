...
public class StatsManager {
    ...
    static final int STATCOLL_INTERVAL_MS = 1000;
     ...
     */
    public StatsManager(XRegionService parent, Logger logger) {
        this.parent = parent;
        this.logger = logger;
        ...
        /* init table stats from metadata */
        parent.getMdMan().getMRTables()
              .forEach(table -> stats.getOrCreateTableMetrics(
                  table.getFullNamespaceName(), ((TableImpl) table).getId()));
    ...
    public static String getStatAgentId(NoSQLSubscriberId sid) {
    ...
    public void shutdown() {
        ...
        try {
            ...
            logger.fine(() -> "Waiting for stat report to complete " +
                              "failed, " + e.getMessage());
        }
        executor.shutdown();
    ...
    public void scheduleStatReport() {
        ...
        logger.info(lm("Schedule stat report task, interval=" + statIntv));
    ...
    private void collectTableMetrics(Set<BaseRegionAgentMetrics> ams,
        ...
        if (stats.getTableMetrics(table) == null) {
            /* table has been removed during collection */
            logger.fine(() -> lm("Table=" + table + " is removed."));
            return;
        }
    ...
    public void collectReportStat() {
        ...
        logger.fine(() -> "Stat collection starts");
        try {
            final long ts = collect();
            if (sysTable == null) {
                sysTable =
                    ServiceMDMan.getTableRetry(tableAPI, TABLE_NAME, 2, logger);
                if (sysTable == null) {
            ...
            logger.finest(() -> "Start report stat to table=" + TABLE_NAME);
            ...
            /* write a row */
            reportStat(row);
            logger.finest(() -> "Agent stats=" + row.toJsonString(true));
            ...
            if (!shutDownRequested) {
                logger.info(lm("Interrupted and exit, " + ie.getMessage()));
            }
        } catch (Exception exp) {
            logger.warning(lm("Unable to report, stat might be missing or " +
                              "incomplete stat=" + stats +
    ...
    private void reportStat(Row row) throws InterruptedException {
        ...
        while (!shutDownRequested) {
            try {
                ...
                final String err = "Unauthorized to write table=" +
                    TABLE_NAME + ", " + ue.getMessage();
                logger.warning(lm(err));
                break;
            }
        }
    ...
    private void operationFailed(Throwable t) {
        logger.warning(lm("Cannot write statistics: " + t.getMessage() +
                          ", call stack\n" +
