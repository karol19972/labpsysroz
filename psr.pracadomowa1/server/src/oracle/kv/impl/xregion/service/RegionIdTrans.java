...
public class RegionIdTrans {
    ...
    RegionIdTrans(ServiceMDMan parent, Logger logger) {
        ...
        parent.getSrcRegionsForMRT()
              .forEach(r -> srcRegions.put(r.getName(), r));
        ...
        logger.fine(lm("local region id mapper ready, local=" + tgtRegionName));
    ...
    public int translate(String source, int rid) {
        ...
        if (entry != null) {
            /* we can translate */
            logger.finest(lm("RegionId " + rid + " from source " + source +
                             " translated to " + entry.getRegionId() +
                             ", region name " + entry.getRegionName()));
        ...
        logger.fine(lm("Translation added for " + regionName +
