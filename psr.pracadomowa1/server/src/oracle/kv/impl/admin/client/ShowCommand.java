        ...
        public ShellCommandResult executeJsonOutput(String[] args, Shell shell)
            ...
            return new ShowTableExecutor<ShellCommandResult>() {
                ...
                public ShellCommandResult
                    ...
                    ObjectNode on = null;
                    try {
                        on = (ObjectNode) JsonUtils.getObjectMapper().
                            readTree(jsonString);
                    } catch (Exception e) {
                        throw new IllegalStateException(
                            "Unable to parse table information: " + e);
                    }
        ...
        private abstract class ShowMRTableAgentStatExecutor<T>
            ...
            private void putStatNode(Row row,
                ...
                String returnValue = row.get(COL_NAME_STATISTICS).
                    toJsonString(false);
                JsonNode returnValueNode =
                    oracle.nosql.common.JsonUtils.createJsonNode(returnValue);
            ...
            private ObjectNode multipleResult(TableAPI tableAPI,
                ...
                Map<String, JsonNode> mergedStat = new HashMap<>();
                ...
                if (merge && (result != null)) {
                    ...
                    result.setAll(JsonUtils.getObjectMapper().
                                  convertValue(mergedStat, ObjectNode.class));
                }
                return result;
            }

            /* Aggregate the stat of multiple agents. */
            private void aggregateStat(Map<String, JsonNode> mergedStat,
                                       Row row,
                                       String agentId) {
                String returnValue = row.get(COL_NAME_STATISTICS).
                    toJsonString(false);
                JsonNode returnValueNode =
                    oracle.nosql.common.JsonUtils.createJsonNode(returnValue);
                Iterator<String> it = returnValueNode.fieldNames();
                ...
                while (it.hasNext()) {
                    ...
                    if (!mergedStat.containsKey(fname)) {
                        ...
                        if (PER_AGENT_STAT_FIELDS.contains(fname)) {
                            ...
                            mergedStat.put(fname, new LongNode(sum));
                        }
                    }
                }
            }
