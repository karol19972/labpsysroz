...
public class CommandUtils {
    ...
    static void validateRepFactor(DatacenterId dcid, int rf,
        ...
        if ((rf < dc.getRepFactor()) && dc.getDatacenterType().isPrimary()) {
            throw new ShellUsageException(
                "Replication factor may not be made smaller for primary" +
                " zones." + Shell.eolt +
                "Current replication factor for zone " + dc.getName() +
                " is " + dc.getRepFactor() + ", requested value was " + rf +
                ".",
                command);
        }
