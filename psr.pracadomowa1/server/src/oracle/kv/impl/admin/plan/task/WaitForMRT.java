...
public class WaitForMRT extends AbstractTask {
    ...
    private NextJob waitForResponse(int taskId, TaskRunner runner) {
        final Response response = plan.getAdmin().getMRTServiceManager().
        ...
        if (response != null) {
            switch (response.getType()) {
                ...
                default :
                    return new NextJob(Task.State.ERROR,
                                       null, null,
                                       "Unknown response from multi-region" +
                                       " table service: " + response);
            }
        }
        plan.getRateLimitingLogger().log(getName(), Level.INFO,
                                         () -> this + " no response, retry in " +
                                         POLL_PERIOD.asString());
    ...
    public StringBuilder getName(StringBuilder sb) {
        return super.getName(sb).append(" for response to ").append(plan.getId());
    }
