...
public class UpdateDatacenter extends SingleJobTask {
    ...
    /**
     * Reducing the replication factor for a secondary zone requires all admins
     * to be at this version.
     */
    private static final KVVersion REDUCE_SECONDARY_RF_VERSION =
        KVVersion.R20_2;

    static {
        assert KVVersion.PREREQUISITE_VERSION.compareTo(
            REDUCE_SECONDARY_RF_VERSION) < 0 :
        "Checks for permitting reducing secondary zone RF can be removed";
    }
    ...
    public static class UpdateDatacenterV2 extends UpdateDatacenter {
        ...
        private void checkVersionRequirements() {
            ...
            final Topology current = admin.getCurrentTopology();
            final Datacenter currentdc = current.get(dcId);
            if ((newRepFactor < currentdc.getRepFactor()) &&
                !admin.checkAdminGroupVersion(REDUCE_SECONDARY_RF_VERSION)) {
                throw new IllegalCommandException(
                    "Cannot reduce the replication factor for a zone until" +
                    " all Admin nodes are at or above software version " +
                    REDUCE_SECONDARY_RF_VERSION.getNumericVersionString());
            }
        }
    }
}
