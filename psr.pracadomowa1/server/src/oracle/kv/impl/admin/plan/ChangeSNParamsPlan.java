...
public class ChangeSNParamsPlan extends AbstractPlan {
    ...
    private void generateRNUpdateTasksV1(Map<RepNodeId, ParameterMap> pmap,
                                         StorageNodeId snId) {
        for (Entry<RepNodeId, ParameterMap>  pval : pmap.entrySet()) {
    ...
    private void updateANParams(StorageNodeId snId,
        ...
        if (admin.checkAdminGroupVersion(WriteNewANParams.WITH_NOTIFY)) {
            generateANUpdateTasksV2(pmap, snId);
        }
        /*
         * No need to add for before V2 since dns cache ttl is not modifiable
         * before.
         */
    }
    ...
    private void updateAdminParams(StorageNodeId snId,
        ...
        if (admin.checkAdminGroupVersion(
                SerialVersion.getKVVersion(
                    SerialVersion.
                    GENERAL_WRITE_NEW_ADMIN_PARAMS_TASK_VERSION))) {
