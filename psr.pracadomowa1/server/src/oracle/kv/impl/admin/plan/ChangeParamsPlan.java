...
public class ChangeParamsPlan extends AbstractPlan {
    ...
    public ChangeParamsPlan(String name,
        ...
        boolean useNewProcessing = false;
        try {
            if (planner.getAdmin().checkAdminGroupVersion(
                            WriteNewParams.WITH_NOTIFY)) {
                useNewProcessing = true;
            }
        } catch (Exception e) {
            /*
             * Ignore problems if you can't get the admin group version and
             * just use old processing.
             */
        }
        ...
        if (useNewProcessing) {
            ...
            generateTasksV1(rnids, topology);
        }
    }
    ...
    private void generateTasksV1( Set<RepNodeId> rnids, Topology topology) {
