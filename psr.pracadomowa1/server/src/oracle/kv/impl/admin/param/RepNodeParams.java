...
public class RepNodeParams extends GroupNodeParams {
    ...
    /**
     * Explicitly enable off-heap cache for unit tests, if the property value
     * is true. default:false
     */
    public static final boolean EXPLICITLY_ENABLE_OFFHEAP_CACHE =
        Boolean.getBoolean("oracle.kv.test.offheap.enable");
     ...
     * memory (if any) after allocating memory for the RN's Heap, JVM overhead
     * and for System use and is calculated as follows:
     *
     *<code>
     * Offheap cache size = (COMMON_MB * (100 - SN_SYSTEM_PERCENT) / 100 -
     *                      RNs Heap size - JVM overhead) / Capacity
     *</code>
     *
     ...
     *
     * If heap size doesn't hit 32 GB limit, the offheap cache is disabled, and
     * system will use all of the available memory for the heap.
     *
    ...
    public long getRNMaxOffHeap(StorageNodeParams snp) {
        ...
        final long maxHeapBytes = getMaxHeapBytes();
        if (!EXPLICITLY_ENABLE_OFFHEAP_CACHE) {
            if (maxHeapBytes < StorageNodeParams.MAX_COMPRESSED_OOPS_HEAP_SIZE) {
                /*
                 * If heap size doesn't hit 32GB limit, disable offheap cache,
                 * and system uses all of the available memory for the heap.
                 */
                return 0;
            }
        }

        final long totalHeapBytes = maxHeapBytes *
            ((snp.getCapacity() != 0) ? snp.getCapacity() : 1);

        final long totalSystemBytes = (snBytes * snp.getSystemPercent()) / 100;

        /* need to account for JVM overhead for residualBytes */
        final int jvmOverheadPercent = snp.getJVMOverheadPercent();
        final long residualBytes =
            snBytes -
            (totalHeapBytes * (jvmOverheadPercent + 100) / 100) -
            totalSystemBytes;

        final long offheapBytesPerRN =
            residualBytes / ((snp.getCapacity() != 0) ? snp.getCapacity() : 1);
    ...
    public String getJvmOverride() {
        return getMap().getOrDefault(ParameterState.JVM_RN_OVERRIDE).asString();
    }
