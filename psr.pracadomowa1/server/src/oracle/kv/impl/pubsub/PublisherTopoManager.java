...
public class PublisherTopoManager {
    ...
    synchronized ShardMasterInfo buildMasterInfo(RepGroupId gid,
        ...
        if (exist != null) {
            ...
            if (prev == null) {
                logger.fine(lm("No previous master, return cached master: " +
                               exist));
                return exist;
            }
            /* already have a newer master, return it */
            if (exist.getTimestamp() > prev.getTimestamp()) {
                logger.fine(lm("Already has a newer master for " + gid +
                               ": " + exist));
                return exist;
            }
        }
        ...
        logger.fine(lm("Need refresh master info for " + gid));
    ...
    private ShardMasterInfo refreshMasterInfo(RepGroupId gid) {
        ...
        ShardMasterInfo ret =
            ShardMasterInfo.buildFromDispatcher(kvs.getDispatcher(),
                                                gid, logger);
        if (ret != null) {
            logger.fine(lm("Refreshed master from request dispatcher for " +
                           gid));
            return ret;
        }
        ...
        if (ret == null) {
            logger.fine(lm("Ping collector is unable locate the master for " +
                           "shard " + gid));
        } else {
            logger.fine(lm("Refreshed master from ping collector for shard " +
                           gid));
        }
        return ret;
    }
