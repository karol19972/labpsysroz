...
abstract class TableStorageHandlerBase<K extends WritableComparable<?>,
    ...
    private static final String THIS_CLASSNAME =
                                    TableStorageHandlerBase.class.getName();
    ...
    private void configureJobProperties(TableDesc tableDesc,
        ...
        LOG2.debug("kvStoreName = " + kvStoreName);
        ...
        LOG2.debug("kvHelperHosts = " + kvHelperHostsStr);
        ...
        LOG2.debug("kvHadoopHosts = " + kvHadoopHostsStr);
        ...
        LOG2.debug("tableName = " + tableName);
        ...
        LOG2.trace("primaryKeyProperty = " + primaryKeyProperty);
        ...
        LOG2.trace("fieldRangeProperty = " + fieldRangeProperty);
        ...
        LOG2.trace("consistency = " + consistencyStr);
        ...
        LOG2.trace("maxRequests = " + maxRequests);
        ...
        LOG2.trace("batchSize = " + batchSize);
        ...
        LOG2.trace("maxBatches = " + maxBatches);
        ...
        /*
         * Fix for JIRA KVSTORE-372:
         *
         * For Big Data SQL, must reset the splits before executing the
         * current query; otherwise the splits from the previous query
         * will be used. If that previous query was on a different table,
         * then the different table info in the previous splits will
         * cause Big Data SQL to declare an error containing a line
         * such as,
         *
         *   KUP-11504: error for external driver:
         *   oracle.hadoop.sql.JXADException: error parsing
         *   "com.oracle.bigdata.colmap" field name "type"
         */
        LOG.trace("reset splits the new table query");
        LOG2.trace("reset splits the new table query");
    ...
    public DecomposedPredicate decomposePredicate(
        ...
        if (analyzer != null) { /* Use TableScan or IndexScan */
            ...
            if (LOG2.isDebugEnabled()) {
                ...
                LOG2.debug("residual = " +
                           decomposedPredicate.residualPredicate);
                LOG2.debug("predicate = " +
                           decomposedPredicate.pushedPredicate);
                LOG2.debug("search conditions = " + searchConditions);

                switch (TableHiveInputFormat.getQueryBy()) {
                    case TableInputSplit.QUERY_BY_INDEX:
                        LOG2.debug("push predicate to secondary index [" +
                                   "WHERE " + whereStr + "]");
                        break;

                    case TableInputSplit.QUERY_BY_PRIMARY_ALL_PARTITIONS:
                    case TableInputSplit.QUERY_BY_PRIMARY_SINGLE_PARTITION:
                        LOG2.debug("push predicate to primary index [" +
                                   "WHERE " + whereStr + "]");
                        break;
                    default:
                        break;
            ...
            if (extendedAnalyzer == null) {
                ...
                LOG2.debug("extended predicate analyzer = null ... " +
                           "NO PREDICATE PUSHDOWN");
                return null;
            }
            ...
            if (LOG2.isTraceEnabled()) {
                final ExprNodeDesc[] qElements =
                    pushPredicateDeque.toArray(
                        new ExprNodeDesc[pushPredicateDeque.size()]);
                LOG2.trace("-----------------------------");
                LOG2.trace("push predicate queue elements:");
                for (int i = 0; i < qElements.length; i++) {
                    LOG2.trace("element[" + i + "] = " + qElements[i]);
                }
                LOG2.trace("-----------------------------");
            }
            ...
            if (decomposedPredicate.pushedPredicate != null) {
                ...
                if (LOG2.isDebugEnabled()) {
                    ...
                    LOG2.debug("predicate = " +
                               decomposedPredicate.pushedPredicate);
                    LOG2.debug("push predicate via native query [" +
                               "WHERE " + whereStr + "]");
                    LOG2.debug("-----------------------------");
                }
                ...
                /*
                 * The pushedPredicate is null; which means that nothing
                 * from the predicate in the query can be sent to the
                 * store for backend filtering.
                 *
                 * If the residualPredicate is also null, then this means
                 * that nothing from the original predicate could be
                 * placed in the residualPredicate either. Hive1 previously
                 * handled a null residualPredicate by applying the whole
                 * original predicate on the client side. But Hive2 handles
                 * this situation differently. When a non-null
                 * decomposedPredicate containing a pushedPredicate and a
                 * residualPredicate that are both null is returned from
                 * this method, then Hive2 seems to now apply a different
                 * code path in which the results sent from the store can be
                 * corrupted; for example, by overwriting the value of a
                 * column in the output with the same, incorrect, value.
                 *
                 * Thus, if the pushedPredicate and the residualPredicate
                 * are both null, then rather than returning a non-null
                 * decomposedPredicate in which both of its predicate fields
                 * are set to null, a null DecomposedPredicate should
                 * instead be returned. This will then cause Hive to
                 * by-pass its predicate pushdown mechanism altogether,
                 * so that all filtering is performed on the client side.
                 */
                if (decomposedPredicate.residualPredicate == null) {

                    LOG.debug(
                        "Predicate analyzer found no predicate to push " +
                        "and no residual predicate; thus, NO PREDICATE " +
                        "PUSHDOWN can be performed. All filtering will " +
                        "be performed on the client side using the " +
                        "original predicate.");
                    LOG2.debug(
                         "Predicate analyzer found no predicate to push " +
                         "and no residual predicate; thus, NO PREDICATE " +
                ...
                /*
                 * The pushedPredicate is null, but the residualPredicate
                 * is non-null. Return the decomposedPredicate so that
                 * Hive will perform all filtering on the client side
                 * using the non-null residualPredicate.
                 */
                LOG.debug("Predicate analyzer found no predicate to " +
                          "push but found a non-null residual predicate. " +
                          "All filtering will be performed on the client " +
                          "side using the non-null residual predicate.");
                LOG2.debug("Predicate analyzer found no predicate to " +
                           "push but found a non-null residual predicate. " +
                           "All filtering will be performed on the client " +
                           "side using the non-null residual predicate.");
            }
