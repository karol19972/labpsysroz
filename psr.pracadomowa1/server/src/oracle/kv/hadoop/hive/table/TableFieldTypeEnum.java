...
public enum TableFieldTypeEnum {
    ...
    TABLE_FIELD_JSON {
        @Override
        public String toString() {
            return FieldDef.Type.JSON.toString();
        }
    },
    TABLE_FIELD_BOOLEAN {
        @Override
    ...
    TABLE_FIELD_NUMBER {
        @Override
        public String toString() {
            return FieldDef.Type.NUMBER.toString();
        }
    },
    TABLE_FIELD_TIMESTAMP {
        @Override
        public String toString() {
            return FieldDef.Type.TIMESTAMP.toString();
        }
    },
    TABLE_FIELD_ENUM {
        @Override
    ...
    public static TableFieldTypeEnum fromHiveType(
        ...
        if (serdeConstants.INT_TYPE_NAME.equals(typeName)) {
            if (TABLE_FIELD_NUMBER.equals(fromKvType(kvType))) {
                return TABLE_FIELD_NUMBER;
            }
        ...
        if (typeName != null &&
            typeName.startsWith(serdeConstants.DECIMAL_TYPE_NAME)) {
            if (TABLE_FIELD_NUMBER.equals(fromKvType(kvType))) {
                return TABLE_FIELD_NUMBER;
            }
        ...
        if (serdeConstants.TIMESTAMP_TYPE_NAME.equals(typeName)) {
            return TABLE_FIELD_TIMESTAMP;
        }
        if (serdeConstants.STRING_TYPE_NAME.equals(typeName)) {
            if (TABLE_FIELD_JSON.equals(fromKvType(kvType))) {
                return TABLE_FIELD_JSON;
            }
    ...
    public static boolean kvHiveTypesMatch(
        ...
        final String THIS_CLASSNAME = TableFieldTypeEnum.class.getName();

        final Log LOG = LogFactory.getLog(THIS_CLASSNAME);
        final Logger LOG2 = LogManager.getLogger(THIS_CLASSNAME);
        ...
        if (!fromKvType(kvFieldType).equals(
            ...
            if (!(TABLE_FIELD_ENUM.equals(fromKvType(kvFieldType)) &&
                ...
                LOG.warn("Field type MISMATCH: " + fromKvType(kvFieldType) +
                         " != " + fromHiveType(hiveColumnType, kvFieldType));
                LOG2.warn("Field type MISMATCH: " + fromKvType(kvFieldType) +
                           " != " + fromHiveType(hiveColumnType, kvFieldType));
                return false;
            }
        }
        ...
        switch (kvFieldType) {
            ...
            case ENUM:
                ...
                if (!TABLE_FIELD_STRING.equals(
                    ...
                    LOG.warn("Field type MISMATCH: for KV ENUM field type, " +
                             "expected Hive STRING column type, but Hive " +
                             "column type is " +
                             hiveColumnType.getTypeName());
                    LOG2.warn("Field type MISMATCH: for KV ENUM field type, " +
                              "expected Hive STRING column type, but Hive " +
                              "column type is " +
                              hiveColumnType.getTypeName());
                    return false;
            ...
            case ARRAY:
                ...
                if (!Category.LIST.equals(hiveColumnType.getCategory())) {
                    ...
                    LOG.warn("Field type MISMATCH: for KV ARRAY field " +
                             "type, expected Hive LIST column type, but " +
                             "Hive column type is " +
                             hiveColumnType.getCategory());
                    LOG2.warn("Field type MISMATCH: for KV ARRAY field " +
                              "type, expected Hive LIST column type, but " +
                              "Hive column type is " +
                              hiveColumnType.getCategory());
                    return false;
                ...
                LOG2.debug("KV ARRAY field type and Hive LIST column type: " +
                           "comparing KV ARRAY element type [" +
                           kvElementDef.getType() + "] with Hive LIST " +
                           "element type [" + hiveElementType + "]");
            ...
            case MAP:
                ...
                if (!Category.MAP.equals(hiveColumnType.getCategory())) {
                    ...
                    LOG.warn("Field type MISMATCH: for KV MAP field type, " +
                             "expected Hive MAP column type, but Hive " +
                             "column type is " +
                             hiveColumnType.getCategory());
                    LOG2.warn("Field type MISMATCH: for KV MAP field type, " +
                              "expected Hive MAP column type, but Hive " +
                              "column type is " +
                              hiveColumnType.getCategory());
                    return false;
                ...
                LOG2.debug("KV MAP field type and Hive MAP column type: " +
                           "comparing KV MAP value type [" +
                           kvMapValueDef.getType() + "] with Hive MAP " +
                           "value type [" + hiveMapValType + "]");
            ...
            case RECORD:
                ...
                if (!Category.STRUCT.equals(hiveColumnType.getCategory())) {
                    ...
                    LOG.warn("Field type MISMATCH: for KV RECORD field " +
                             "type, expected Hive STRUCT column type, but " +
                             "Hive column type is " +
                             hiveColumnType.getCategory());
                    LOG2.warn("Field type MISMATCH: for KV RECORD field " +
                              "type, expected Hive STRUCT column type, but " +
                              "Hive column type is " +
                              hiveColumnType.getCategory());
                    return false;
            ...
            default:
                LOG.warn("Field type MISMATCH: UNKNOWN KV field type " +
                         "[" + kvFieldType + "]");
                LOG2.warn("Field type MISMATCH: UNKNOWN KV field type " +
                          "[" + kvFieldType + "]");
                return false;
        }
    }
