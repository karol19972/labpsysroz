...
public class SNRecover extends AdminSNRecover {
    ...
    private Map<ShardName,NodeJDBFiles> winnerNodesJDBFilesMap =
        new HashMap<>();
    ...
    private void deserializeRequiredFileJSON(File requiredFileJSON)
        ...
        Map<ShardName,NodeJDBFiles> shardNodeJDBFilesMap;

        try {
            TypeReference<HashMap<ShardName, NodeJDBFiles>> typeRef =
                new TypeReference<HashMap<ShardName, NodeJDBFiles>>() {};
            shardNodeJDBFilesMap = mapper.readValue(requiredFileJSON, typeRef);
        } catch (IOException e) {
            throw new IOException("Exception in deserializing file " +
                                  requiredFileJSON + ": " + e, e);
        }

        for (Map.Entry<ShardName,NodeJDBFiles> entry :
            shardNodeJDBFilesMap.entrySet()) {
            final ShardName shardName = entry.getKey();
    ...
    private void deserializeTopologyOutputJSON(File topologyFileJSON)
        ...
        try {
            topoOutputJSON =
                mapper.readValue(topologyFileJSON, TopologyOutputJSON.class);
        } catch (IOException e) {
            throw new IOException("Exception in deserializing file " +
     ...
     * between the input and output of JSON. To keep TopologyJSON and
     * TopologyPrinter in sync, we will consider re-writing TopologyPrinter
     * to use ObjectMapper.
     */
