...
public class KVLite {
     ...
     * -jmx, -storagedir, -storagedirsize, -numsns, -capacity and -repfactor
     */
    ...
    public void shutdownStore(boolean force) {
        ...
        if (!singleRNfile.exists() && multiRNfile.exists()) {
            ...
            System.err.println("Error configuration file for" +
                    " root: " + kvroot);
        }
        ...
        for(int i = 0; true; i++) {
            ...
            if (!configfile.exists()) {
                if (i == 0) {
                    System.err.println("Cannot find configuration file for" +
                                       " root: " + kvroot);
                }
                break;
            }
            try {
                ...
                System.err.println
                    ("Exception in shutdown, maybe the service is not " +
    ...
    private void initStorageNodes()
        ...
        if (repfactor <= 0 || capacity <= 0 || numStorageNodes <= 0) {
            throw new Exception("ERROR: repfactor, capacity and numStorageNodes"
                    + " should not be 0");
        }
        if (repfactor > numStorageNodes) {
            throw new Exception("ERROR: repfactor should not be larger than"
                    + "numStorageNodes");
        }
        if(port == null || port.length != numStorageNodes) {
            throw new Exception("ERROR: port number is not equal to "
                    + "numStorageNodes " + port.length + " vs "+ numStorageNodes);
        }
        if (haPortRange != null && haPortRange.length != numStorageNodes) {
            throw new Exception("ERROR: haPortRange number is not equal to "
                    + "numStorageNodes");
        }
        if (servicePortRange != null && 
            servicePortRange.length != numStorageNodes) {
            throw new Exception("ERROR: servicePortRange number is not equal to"
                    + " numStorageNodes");
        }
            ...
            if (registered == snas.length) {
                if (verbose) {
                    ...
                    System.out.println
                        ("Opened existing kvlite store with config:\n" +
                         ...
                         snas[0].getStorageNodeAgent().getHostname() + " " +
                ...
                if (waitForServices) {
                    ...
                    if (isSecure || runBootAdmin || !singleRN) {
                        ...
                        admin = ServiceUtils.waitForAdmin(host, port[0],
                                loginManager, 10, ServiceStatus.RUNNING);
                    }
                ...
                throw new Exception("KVLite: not all SNA registered");
            }
        } catch (Exception e) {
